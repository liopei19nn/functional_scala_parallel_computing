...
object LineOfSight {
	...
	def lineOfSight(input: Array[Float], output: Array[Float]): Unit = {
		def helper(idx: Int, maximum: Float, len: Int): Unit = {
			if (idx < len) {
				val tangent = input(idx) / idx
				output(idx) = max(maximum, tangent)
				helper(idx + 1, output(idx), len)
			}
		}
		output(0) = 0
		helper(1, 0, input.length)
	}
	...
	def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
		@tailrec
		def helper(idx: Int, maximum: Float): Float = {
			if (idx >= until) maximum
			else helper(idx + 1, max(input(idx) / idx, maximum))
		}
		helper(from, 0)
	}
	...
	def downsweepSequential(input: Array[Float], output: Array[Float],
		...
		def helper(idx: Int, maximum: Float): Unit = {
			if (idx < until) {
